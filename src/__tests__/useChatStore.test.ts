...
vi.mock('../config/ai.config', () => ({
  AIServiceFactory: {
    create: vi.fn(() => ({
      streamMessage: vi.fn(async (_messages: unknown, cb: (t: string) => void) => {
        cb('Streamed content');
...
describe('useChatStore', () => {
  ...
  it('should add a message and call FirebaseChatRepository', () => {
  ...
  it('should send message stream via AI service and update Firebase with final content', async () => {
    await useChatStore.getState().sendMessageStream('How are you?');

    const state = useChatStore.getState();
    // initial + user msg + assistant msg
    expect(state.messages).toHaveLength(3);
    expect(FirebaseChatRepository.addMessage).toHaveBeenCalled();
  });
